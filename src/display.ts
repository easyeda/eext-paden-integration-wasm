import type { SolutionData, AnalysisImages, PcbContextData, NetworkInfo, AnalysisResultSet } from './types';
...
export class ResultDisplay {
  ...
  static cleanupStorage(): void {
    try {
      ...
      console.log('[Display] Storage cleaned up');
    } catch (e) {
  ...
  showResultSet(
    ...
    return new Promise((resolve) => {
      ...
      console.log('[Display] Storage write: data size =', jsonStr.length, 'chars, results =', resultSet.results.length);
      try {
      ...
      const task = eda.sys_MessageBus.subscribe('padne-results-ready', () => {
        task.cancel();
        console.log('[Display] Received padne-results-ready, sending data via message bus');
