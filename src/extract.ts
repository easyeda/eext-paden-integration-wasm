...
export class PcbExtractor {
	...
	async extractNetworkInfo(onProgress?: (percent: number) => void): Promise<EasyEDA_PcbData> {
		...
		const [bulkVias, bulkPads, bulkTracks] = await Promise.all([
			(async () => {
				try {
					return await eda.pcb_PrimitiveVia.getAll();
				}
				catch { return null; }
			})(),
			(async () => {
				try {
					return await eda.pcb_PrimitivePad.getAll();
				}
				catch { return null; }
			})(),
			(async () => {
		...
		try {
			...
			const compMeta = components.map(comp => ({
				refDes: typeof comp.getState_Designator === 'function'
					? comp.getState_Designator()
					: undefined,
				deviceName: typeof comp.getState_OtherProperty === 'function'
					? (comp.getState_OtherProperty()?.Device as string | undefined)
					: undefined,
	...
	private getNetFromPrimitive(primitive: any): string | undefined {
		try {
			...
			if (typeof netObj === 'string')
				return netObj.trim() || undefined;
			if (netObj && typeof netObj === 'object') {
				if (typeof netObj.name === 'string')
					return netObj.name.trim() || undefined;
	...
	private extractPad(primitive: any, refDes?: string, deviceName?: string, fallbackNet?: string): EasyEDA_Pad | null {
		try {
			...
			try {
				...
				if (Array.isArray(hole) && typeof hole[1] === 'number')
					holeD = hole[1];
			}
			catch {}
			...
			if (!netName) {
				try {
					...
					if (typeof netObj === 'string')
						netName = netObj;
					else if (netObj && typeof netObj.name === 'string')
						netName = netObj.name;
					else if (netObj && typeof netObj.getName === 'function')
						netName = netObj.getName();
				}
				catch {}
			}

			if (x === null || y === null || !netName)
				return null;
			return {
				net: netName,
				x,
				...
				try {
					...
					if (pourFills.length === 0) {
						...
						if (pourObj) {
							...
							if (pourSrc)
								this.diag(`  Pour边框 source preview: ${JSON.stringify(pourSrc).substring(0, 500)}`);
	...
	private parsePourFill(fill: any, logDetail: boolean): {
		...
		if (fill && fill.path) {
			...
			if (logDetail && src)
				this.diag(`  pourFill.path source preview: ${JSON.stringify(src).substring(0, 500)}`);
				...
				if (src.length > 0 && Array.isArray(src[0])) {
					for (let si = 0; si < src.length; si++) {
						...
						if (si === 0) {
							...
							if (subParsed.length >= 3)
								holes.push(subParsed);
						}
					}
				}
				else {
