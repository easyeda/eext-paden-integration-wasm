...
export class PcbDataConverter {
	...
	private detectGroundNet(data: EasyEDA_PcbData, exclude: Set<string>): string | undefined {
		const nets: string[] = [];
		const add = (net: string) => {
			if (net)
				nets.push(net);
		};
		data.pads.forEach(p => add(p.net));
		data.vias.forEach(v => add(v.net));
		data.tracks.forEach(t => add(t.net));
		data.copperPours.forEach(c => add(c.net));
		...
		const isGroundName = (name: string): boolean => {
			const ln = name.toLowerCase();
			const exact = ['gnd', 'ground', '0', 'earth', 'vss', 'vee', 'agnd', 'dgnd', 'sgnd', 'pgnd'];
			if (exact.includes(ln))
				return true;
			return ln.includes('gnd') || ln.includes('ground') || ln.includes('vss');
		};
		...
		for (const net of nets) {
			...
			if (!isGroundName(net))
				continue;
			const count = counts[net.toLowerCase()] ?? 0;
			if (count > bestCount) {
				best = net;
				bestCount = count;
			}
	...
	buildGerberConfig(
		...
		for (const rail of config.rails) {
			...
			if (rail.gnd_net)
				analysisNets.add(rail.gnd_net);
		}

		// Auto-detect ground net when the user did not explicitly configure one.
		// This is required for PDN analysis: the solver needs a return/reference net.
		const sourceNets = new Set(config.rails.map(r => r.net));
		const detectedGndNet = this.detectGroundNet(easyedaData, sourceNets);
		const gndNet = config.rails.find(r => r.gnd_net)?.gnd_net ?? detectedGndNet ?? '';
		if (gndNet)
			analysisNets.add(gndNet);
		...
		const sortedLayerNames = sortedLayerEntries.map(entry => entry[1]);
		...
		const parseViaLayerRange = (viaType: string | undefined): string[] | null => {
			if (!viaType)
				return null; // Through-hole via (all layers)
			...
			const blindMatch = viaType.match(/Blind:\s*([^\s-]+)\s*-\s*([^\s-]+)/);
			const buriedMatch = !blindMatch && viaType.match(/Buried:\s*([^\s-]+)\s*-\s*([^\s-]+)/);
