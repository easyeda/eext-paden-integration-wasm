...
async function main() {
	...
	const fixture = await fixturePoints(path.join(__dirname, 'test-4.tgz'), [NET, 'GND'], layerNames);
	...
	if (netPoints.length < 2 || gndPoints.length === 0)
		throw new Error('ODB++ fixture does not contain enough +3.3VA/GND features');
	const sourcePads = [netPoints[0]];
	const loadPads = [netPoints[netPoints.length - 1]];
